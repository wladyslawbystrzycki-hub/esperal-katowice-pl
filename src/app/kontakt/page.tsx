...
import { getKontaktContent } from "@/lib/content/kontakt";

export async function generateMetadata(): Promise<Metadata> {
  const site = await getCurrentSite();
  const content = getKontaktContent(site.key);
  return {
  ...
  return (
    <>
      ...
      <Section className="contact-intro bg-white">
        ...
        <div className="contact-intro__text space-y-5 leading-relaxed text-neutral-600">
        ...
        <div className="contact-cards__grid grid gap-6 md:grid-cols-2">
          ...
          <article className="contact-card contact-card--phone rounded-lg bg-gradient-to-br from-neutral-950 to-neutral-800 p-7 text-white shadow-[0_18px_40px_-20px_rgba(0,0,0,0.5)]">
            ...
            <p className="contact-card__description mb-5 text-sm text-white/80">
              ...
              codziennie w godzinach pracy gabinetu. Poza godzinami skorzystaj z{" "}
              <Link
                href="/#e-rejestracja"
            ...
            <div className="mt-6 flex items-start gap-3 border-t border-white/10 pt-5 text-sm text-white/80">
              <span className="mt-0.5 text-primary-500" aria-hidden>
                <MapPinIcon />
              </span>
              <address className="not-italic leading-relaxed">
                <span className="block font-semibold text-white">Gabinet</span>
                {site.streetAddress}
                <br />
      ...
      <Section className="contact-closing bg-white">
        <div className="contact-closing__inner text-center">
          <p
            className="text-lg leading-relaxed text-neutral-600"
            dangerouslySetInnerHTML={{ __html: content.closingHtml }}
          />
          <div className="mt-8">
            <Link
              href="/#e-rejestracja"
